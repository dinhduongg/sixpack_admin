...
export default function EditForm({ basic }: EditFormProps) {
  ...
  const form = useForm<BasicDto>({
    ...
    defaultValues: {
      shell: basic.shell,
      value: basic.value,
      value_en: basic.value_en,
      group: basic.group || undefined,
    },
  })
  ...
  const onSubmit = async (data: BasicDto) => {
    try {
      ...
      const res = await basicApiRequest.update(basic.id!, data)
